...
function workflow(study: Study): void {
  ...
  const draw = (): void => {
    ...
    if (step === "training") { const next = document.createElement("button"); next.className = "primary continue-training"; next.textContent = "Continue"; next.hidden = true; stage.append(next); const enable = (): void => { const status = document.querySelector<HTMLElement>("#status"); if (status && /Progress: (exited|completed|completed-or-unknown|dead)/.test(status.textContent || "")) next.hidden = false; else window.setTimeout(enable, 1000); }; next.onclick = () => { step = "results"; draw(); }; window.setTimeout(enable, 1000); }
    document.querySelector<HTMLButtonElement>("#next")?.addEventListener("click", () => { step = "training"; draw(); });
    document.querySelector<HTMLButtonElement>("#evaluate")?.addEventListener("click", async () => { const output = document.querySelector<HTMLElement>("#evaluation-status")!; const audioResults = document.querySelector<HTMLElement>("#audio-results")!; output.hidden = false; output.textContent = `Loading comparison report for ${runId || "latest"}…`; try { const pointer = await (await fetch("/reports/comparisons/latest.json")).json() as { run_id: string }; const manifest = await (await fetch(`/reports/comparisons/${pointer.run_id}/manifest.json`)).json() as { examples: Array<{ id: string; reference: string; baseline: string; aligned: string }> }; const example = manifest.examples[0]; if (!example) throw new Error("No audio examples are available."); audioResults.innerHTML = `<h3>Playable audio · ${example.id}</h3>${([['Reference', example.reference], ['Baseline', example.baseline], ['Aligned', example.aligned]] as const).map(([label, path]) => `<label class="audio-item">${label}<audio controls preload="metadata" src="/reports/comparisons/${pointer.run_id}/${path}"></audio></label>`).join("")}`; output.textContent = `Evaluation artifacts loaded from MLflow comparison run ${pointer.run_id}.`; } catch (error) { output.textContent = error instanceof Error ? error.message : "Unable to load evaluation artifacts."; } });
